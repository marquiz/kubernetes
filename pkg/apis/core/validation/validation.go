...
func ValidatePodSpecificAnnotationUpdates(newPod, oldPod *core.Pod, fldPath *field.Path, opts PodValidationOptions) field.ErrorList {
	...
	for k, oldVal := range oldAnnotations {
		...
		if strings.HasSuffix(k, v1.QoSResourceDefaultAnnotationKeyBase) || strings.Index(k, v1.QoSResourceContainerAnnotationPrefixBase) > -1 {
			allErrs = append(allErrs, field.Forbidden(fldPath.Key(k), "may not remove or update QoS resource annotations"))
		}
	}
	// Check for additions
	for k := range newAnnotations {
		...
		if strings.HasSuffix(k, v1.QoSResourceDefaultAnnotationKeyBase) || strings.Index(k, v1.QoSResourceContainerAnnotationPrefixBase) > -1 {
			allErrs = append(allErrs, field.Forbidden(fldPath.Key(k), "may not add QoS resource annotations"))
		}
	}
