		...
		cache.ResourceEventHandlerFuncs{
			...
			UpdateFunc: func(old, cur interface{}) {
				...
				if quota.Equals(oldResourceQuota.Spec.Hard, curResourceQuota.Spec.Hard) &&
					quota.ClassResourceEquals(oldResourceQuota.Spec.ClassResources, curResourceQuota.Spec.ClassResources) {
					return
				}
...
func (rq *Controller) syncResourceQuota(ctx context.Context, resourceQuota *v1.ResourceQuota) (err error) {
	...
	statusLimitsDirty = statusLimitsDirty || !apiequality.Semantic.DeepEqual(resourceQuota.Spec.ClassResources, resourceQuota.Status.ClassResources)
	...
	// Copy class resources as is, we don't track the actual usage stats
	classResources := make([]v1.ClassResourceInfo, 0, len(resourceQuota.Spec.ClassResources))
	for i := range resourceQuota.Spec.ClassResources {
		classResources = append(classResources, *resourceQuota.Spec.ClassResources[i].DeepCopy())
	}
